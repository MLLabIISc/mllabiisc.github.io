...
- [x] Menu (your request, updated 2026-09-12): About · Publications · Projects · People · More ▾ (News, Awards & Press, Gallery, ─, Opportunities). The Research dropdown was removed (`_pages/research.md` deleted); Publications and Projects are top-level again. The only dropdown is `_pages/more.md` (al-folio's `dropdown: true` + `children:`); the pages inside it have `nav: false`.
...
- [x] **Home page layout (your request, 2026-09-12):** the order is now intro text → **Recent Publications** (renamed from "selected publications"; still the 5 papers marked `selected`) → **Our Partners** (renamed from "Our Collaborators") → social icons and contact line. **News is switched off** on the home page (`announcements.enabled: false`); its heading is now "News" in case you turn it back on. The News page under More ▾ is unchanged. Partner logos moved from the page text into a `partners:` list in `_pages/about.md` front matter, rendered by `_includes/partners.liquid`; the order and headings are in `_layouts/about.liquid`.
...
## News (`_news/`, 11 items)
- [x] **Paper announcements removed (your request, 2026-09-12):** 21 items that announced a paper (the ones beginning "The paper …" / "A paper …", 2018–2026) were deleted. The 11 remaining are awards, fellowships, workshops, the CSA Chair appointment, the Tata Motors collaboration, the XraySetu launch, the ICSR award and the Karnataka task group. Some notes below refer to deleted items and no longer apply.
...
- [x] **People restructure (your request, 2026-09-12):**
  - M.Tech is now only **Sahil Chaudhary**, with a homepage link and no email, as you asked. The "Jointly advised by Anant Raj" line was removed at your request.
  - The other 10 M.Tech students moved to Alumni (Masters) with **2024–2026**, the range you gave.
  - **Pranav K Nayak** moved to M.Tech (Research).
  - Dhruva now shows 2022–2024 and Tanay 2020–2022; the list note now says years are joining, or joining–graduating where known.
  - **Consistency:** a People-page note says everyone is in CSA unless noted otherwise, so "CSA" was removed from individual entries. The Robert Bosch Centre is still named where it applies (Sayantan, Chaitanya).
  - Roles follow one pattern: "PhD student (since 2018)", "M.Tech (Research) student, ERP (AMD)", "PhD student, sponsored by DRDO" (sponsor notes kept, as you asked). Everyone else is a regular student.
  - Joint advisors read "Jointly advised by …" without naming Prof. Bhattacharyya: Aadirupa (Aditya Gopalan), Shikhar (Partha Pratim Talukdar and Manaal Faruqui).
- [ ] **PhD alumni: current job + first position, and where each came from (looked up 2026-09-12):**
  - Aadirupa Saha: Asst. Prof., UIC (since Fall 2025); first: postdoc, Microsoft Research NYC. Source: aadirupa.github.io.
  - Achintya Kundu: Principal AI/ML Scientist, HP AI Research (Jan 2025; his page says "HP Inc.", and you confirmed he is at HP AI Research with Chaitanya); first: Research Scientist, IBM Research (Aug 2021). Source: achikundu.github.io.
  - Shikhar Vashishth: Senior Research Scientist, Google DeepMind (his LinkedIn headline, via web search; research.google also lists him); first: postdoc, CMU LTI (old site). His github.io homepage returns 404.
  - Lavanya Sita Tekumalla: Machine Learning Scientist, Amazon (lavanyats.github.io, possibly out of date). ⚠️ First position is not stated explicitly; her page's earliest post-PhD role is Amazon from 2017, so I used "Amazon (2017)". The old site called her the founder of MachineLearningInterview.com; that's now dropped.
  - Adway Mitra: Asst. Prof., Dept. of AI, IIT Kharagpur (since 2019); first: Airbus Prize Postdoctoral Fellow, ICTS (2016–2018). Source: web search (IIT KGP and ICTS pages). He was also at IIT Bhubaneswar 2018–2019.
  - Sahely Bhadra: Assoc. Prof., IIT Palakkad (since Jul 2023); first: postdoc, MPI for Informatics & Saarland University (2012–2014). Source: iitpkd.ac.in.
  - Mrinal Kanti Das: Assoc. Prof., Data Science, IIT Palakkad (nmrinl.github.io); first: postdoc, UMass Amherst (old site; his page doesn't say).
  - Saketha Nath Jagarlapudi (renamed from the old site's "Saketh Nath J"; you confirmed he is a Professor at IIT Hyderabad): Professor, CSE, IIT Hyderabad (Apr 2026); ⚠️ first position per his page: "Research Assistant, IISc (Mar–Apr 2009)", which looks like a short stint, so please check.
  - Sourangshu Bhattacharya: Assoc. Prof., CSE, IIT Kharagpur; first: Scientist, Yahoo! Labs (his page, no years).
  - ⚠️ Raman Sankaran: Staff Software Engineer, Google (since 2023), from a web search summary of LinkedIn, **unverified**. "First position: Senior Applied Research Scientist, LinkedIn" comes from the old site's "current role" and is assumed to be his first job.
  - ⚠️ Goutham Tholpadi: **current job unverified.** I kept the old site's "Peritus.ai". A search pointed to Amazon (Seattle), but the same result wrongly said "Cornell PhD", so I didn't trust it. First position: Microsoft Research (old site).
  - ⚠️ Chaitanya Murti: Research Scientist, HP AI Research (you confirmed; same lab as Achintya). **First position not found**, so none is shown.
...
- [ ] **Possibly graduated:** check the M.Tech (Research) and PhD lists against who is still in the lab. Also check the PhD list (e.g. Nabanita, started 2018; Mariamma, started 2019). I did not move anyone to alumni.
...
## Publications (`_bibliography/papers.bib`: 139 → 126 entries)
- [ ] **Added**, with sources:
...
- [x] **Renamed duplicates removed (your request, 2026-09-12):** 5 entries were the same paper listed twice under different names, so they are deleted (131 → 126). For each, the kept published entry gets the preprint's arXiv link.
  - "On the Unreasonable Effectiveness of Knowledge Distillation…" (arXiv 2003.13438) = CIKM 2022 "Analysis of Knowledge Transfer in Kernel Regime". arXiv shows the new title and lists it as published at CIKM 2022.
  - "Graph Convolutional Networks based Word Embeddings" (arXiv 1809.04283) = ACL 2019 "Incorporating Syntactic and Semantic Information in Word Embeddings using GCNs". arXiv shows the new title and lists ACL 2019.
  - "Reliable quantum kernel classification using fewer circuit evaluations" = "Shot-frugal and Robust quantum kernel classifiers": the **same arXiv ID** (2210.06971) under its old DBLP title; "Shot-frugal" is kept.
  - "Temporally Coherent Chinese Restaurant Process…" (arXiv 1409.6080, retitled on arXiv in Feb 2015) = SDM 2015 "Temporally Coherent CRP…". Same authors and work; arXiv lists no venue, so this one rests on content, not a journal reference.
  - "Algorithms for finding k in k-means" (arXiv 2012.04388) = SODA 2022 "How many Clusters? – An algorithmic answer". Same authors; the abstract poses the same questions and uses the same "no tight sub-cluster" (NTSC) condition. arXiv lists no venue.
  - **Kept on purpose (your call):** 9 conference papers that also have a later journal version, e.g. NIPS'00 → JAIR'01 (mean field), NIPS'01 → JMLR'02 (MPM), WABI'02 → Signal Proc.'03, NIPS'04 → JMLR'06 (SOCP/missing data), ICML'10 → JMLR'12 (robust kernels), NIPS'12 → JMLR'13 (Lovász ϑ), CICLing'15 → TALLIP'17, IJCAI'15 / SDM'15 → TPAMI'17 (videos), NIPS'07 ~ arXiv'09 (randomized SVMs). Also kept: similar-looking but different papers (the two SMO papers, the protein-structure papers, Controlled vs Variable Sparsity Kernel Learning).
...
- [x] **Filter (redesigned 2026-09-12, your request):** the Publications page has one filter panel. It has **Area**, **Line of work** and **Venue** dropdowns, with the text search box below them (moved there at your request) (the "(arXiv)" suffix is gone; hovering over an area tag still shows its full arXiv name). Active filters appear as removable chips, next to a result count and a **Reset** button. Reset now has its own style: it highlights in the theme colour on hover instead of greying out (the grey came from al-folio's `.btn` hover), and it's disabled when nothing is active. It also clears the text search.
  - **Adaptive counts:** each option's number is how many publications you'd get if you picked it, given the other active dropdowns **and** the text search. Options that would give zero are greyed out and can't be picked. "All (n)" shows the total under the other filters. Option order stays fixed (most common first), so the lists don't jump around.
  - Matching is exact; clicking a tag under a paper sets that filter; year headings with no visible papers are hidden. Tags appear only on the Publications page (a site-wide rule in `_sass/_base.scss` hides them elsewhere).
  - Files: `_includes/bib_filter.liquid` (markup and styles, using theme colours, so it works in dark mode), `assets/js/bibfilter.js`, the tag row in `_layouts/bib.liquid`, and the tag fields hidden from displayed BibTeX in `_config.yml`.
  - ⚠️ These checks read the built HTML only; nobody has clicked through the filter in a real browser yet.
  - [x] **Bug fixed (2026-09-12, reported by you):** clicking an Area tag that wasn't the first on a paper showed 0 results. The same was true for any second line-of-work tag. Cause: the tag loops in `_layouts/bib.liquid` tried `{% assign tag = tag | strip %}` on the loop variable, which Liquid ignores, so every non-first tag carried a leading space (`data-value=" stat.ML"`; 121 buttons affected), and its hover title missed the arXiv name. Fixed by stripping into a separate variable. `bibfilter.js` also trims clicked values now. A regression test clicks non-first area and line-of-work tags.
